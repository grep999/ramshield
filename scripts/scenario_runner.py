...
Usage:
  ...
  python3 scripts/scenario_runner.py run --file scenarios_99.json --mode wave --chaos
...
# Helper: send events via attack scripts
def run_attack_scenario(profile: Dict[str, Any], target_host: str, target_port: int, workers: int, dry_run: bool = False) -> Tuple[Dict[str, Any], float]:
    """Run a single scenario using attack_nexus or other tools.
    
    Returns (raw_stats dict, elapsed_seconds).
    """
    # Determine if profile has "profile" field referencing a known attack profile
    # Use attack_nexus with mode, but for simplicity call the existing nexus script with args.
    # We'll rely on subprocess.
    import subprocess
    from pathlib import Path
    ...
    cmd = [
        ...
        "--workers", str(workers),
        "--target", profile.get("target_ip", "10.255.0.99"),
        "--profile", profile.get("profile", "l7_http_flood"),
    ]
    if dry_run:
        print(f"[DRY] Would run: {' '.join(cmd)}")
        return {"events": 0, "batches": 0, "errors": 0}, 0.0

    start = time.perf_counter()
    result = subprocess.run(cmd, capture_output=True, text=True)
    elapsed = time.perf_counter() - start
    # attack_nexus prints human summary; we'd need to parse JSON output from dashboard, but for now we just rely on scenario metrics.
    # This is a placeholder; real implementation would fetch dashboard snapshot after scenario.
    return {"events": 0, "batches": 0, "errors": 0}, elapsed
...
# Helper: execute attack script for a scenario
def run_scenario_attack(profile: str, target_host: str, target_port: int, workers: int, dry_run: bool = False) -> Tuple[Dict[str, Any], float]:
"""Run a single scenario using attack_nexus script.
    
Returns (raw_stats dict, elapsed_seconds).
"""
# Prepare attack_nexus command based on scenario profile
# If scenario has profile key, use it; otherwise default to l7_http_flood
attack_profile = profile if profile else "l7_http_flood"
...
cmd = [
    ...
    "--port", str(target_port),
    "--workers", str(workers),
    "--target", "10.255.0.99",  # Use default target; could be scenario-specific
    "--profile", attack_profile,
    "--duration", "30",  # Default duration; could be scenario-specific
]
...
try:
    ...
    elapsed = time.perf_counter() - start
    # Parse attack_nexus output for events sent
    # For simplicity, we'll rely on dashboard snapshot for metrics
    return {"events": 0, "batches": 0, "errors": 0, "output": result.stdout}, elapsed
except Exception as e:
    elapsed = time.perf_counter() - start
    return {"events": 0, "batches": 0, "errors": 1, "error": str(e)}, elapsed
