...
// ============================================================================
// STALE — this Rust aya-ebpf program is NOT the shipping XDP program.
//
// Why: build.rs requires `bpf-linker` (an LLVM 21 tool) to compile this file
// to BPF.  The host ships LLVM 18 (Ubuntu noble), so bpf-linker cannot build.
// build.rs falls back to clang on `main.c`, which IS the shipping program.
//
// The C program (main.c) and this Rust file have DIVERGED:
//   - C: BLOCKLIST map uses __u64[2] key, key[0] = saddr (LE round-trip).
//   - Rust: BLOCKLIST map uses HashMap<u32, u8> key (u32, not __u64[2]).
//   - C: BLOCKLIST6 is a separate __u64[2] map with memcpy key.
//   - Rust: BLOCKLIST6 is HashMap<[u8; 16], u8>.
//
// These layouts are NOT wire-compatible.  This program MUST NOT be loaded
// into a live kernel until the types are reconciled and bpf-linker builds.
//
// DO NOT edit this file for production changes — edit main.c instead.
// ============================================================================
