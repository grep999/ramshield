...
impl TrafficCounters {
    ...
    pub fn record_flush(&self, total_events: u64, unique_ips: u64, subnet_counts: &[u64]) {
        self.events_last_second.store(total_events, Ordering::Relaxed);
...
impl Store {
    ...
    pub fn merge_subnet_window(&self, key: u32, prefix: [u8; 3], events: u32, now_ns: u64) {
        let mut rec = self.subnet_table.get(&key).map(|e| e.value().clone()).unwrap_or(SubnetRecord {
            prefix,
            active_ips: 0,
            blocked_ips: 0,
            total_rps: 0,
    ...
    pub fn insert(
        ...
        if old_size == 0 && current + net_growth > ram_limit_bytes {
            ...
            return Err(RsError::CapacityExceeded { limit_mb: ram_limit_bytes / (1024 * 1024) });
        }
    ...
    pub fn increment(&self, key: &str, delta: u64) -> u64 {
        let mut e = self.inner.entry(key.to_string()).or_insert_with(|| Entry {
            value: Value::Counter(0), expires_at: None, version: 0,
        });
        if let Value::Counter(ref mut c) = e.value { *c += delta; return *c; }
        0
    }

    pub fn evict_batch(&self, keys: &[String]) {
        for key in keys {
            let expired = self.inner.get(key).map_or(false, |e| e.is_expired());
            if expired {
    ...
    pub fn ram_bytes(&self)    -> usize { self.ram_bytes.load(Ordering::Relaxed) }
    pub fn inner(&self)        -> &DashMap<String, Entry>         { &self.inner }
    pub fn subnet_table(&self) -> &DashMap<u32, SubnetRecord>     { &self.subnet_table }
...
mod tests {
    ...
    fn insert_get_remove() {
        ...
        store.insert("k".into(), Value::Counter(1), None, 64 * 1024 * 1024).unwrap();
    ...
    fn ttl_lazy_expiry() {
        let store = Store::new(16);
        store.insert("x".into(), Value::Counter(1), Some(0), 64 * 1024 * 1024).unwrap();
