...
pub enum WalEntry {
    BlockIp   { ip: String, reason: String, ttl_secs: Option<u64>, ts_ns: u64 },
    UnblockIp { ip: String, ts_ns: u64 },
    Insert    { key: String, value_json: String, ttl_secs: Option<u64>, ts_ns: u64 },
    Delete    { key: String, ts_ns: u64 },
    Checkpoint { snapshot_path: String, ts_ns: u64 },
}
...
impl Wal {
    ...
    pub fn append(&self, entry: &WalEntry) -> Result<()> {
        ...
        let (payload, flags): (Vec<u8>, u8) =
            if self.compress && raw.len() > 64 {
                (compress_prepend_size(&raw), 0x01)
            } else {
                (raw, 0x00)
            };
        let mut h = Crc32::new(); h.update(&payload);
        let crc = h.finalize();
    ...
    pub fn replay(dir: &str) -> Result<Vec<WalEntry>> {
        let mut segs: Vec<PathBuf> = match std::fs::read_dir(dir) {
            Ok(rd) => rd.filter_map(|e| e.ok()).map(|e| e.path())
                .filter(|p| p.extension().map_or(false, |x| x == "rshw"))
                .collect(),
        ...
        for seg in &segs {
            ...
            while pos + HEADER <= data.len() {
                ...
                if pos + HEADER + plen > data.len() { warn!("truncated at {}", pos); break; }
                let payload = &data[pos+HEADER..pos+HEADER+plen];
                let mut h = Crc32::new(); h.update(payload);
                if h.finalize() != crc { return Err(RsError::CorruptWal { offset: pos as u64 }); }
                let decoded: Vec<u8> = if flags & 0x01 != 0 {
                    decompress_size_prepended(payload).map_err(|e| RsError::Serde(e.to_string()))?
                } else {
                    payload.to_vec()
                };
                let entry: WalEntry = serde_json::from_slice(&decoded)
...
mod tests {
    ...
    #[test]
    fn wal_roundtrip() {
        let dir = std::env::temp_dir().join("rs_wal_rt").to_string_lossy().to_string();
        let _ = std::fs::remove_dir_all(&dir);
        let wal = Wal::open(&dir, true, "none", 64 * 1024 * 1024).unwrap();
        wal.append(&WalEntry::BlockIp {
            ip: "1.2.3.4".into(), reason: "test".into(), ttl_secs: Some(60), ts_ns: 1,
        }).unwrap();
        drop(wal);
